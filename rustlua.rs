...
//	fn lua_getglobal(L: LuaState, name: *c_char); <- C MACRO, is lua_getfield(L, LUA_GLOBALSINDEX, s)
...
//	fn lua_pushliteral(L: LuaState, s: *c_char) -> c_void;	<- C MACRO, equal to lua_pushstring(L,s)
...
//	fn lua_register(L: LuaState, name: *c_char, f: LuaCFunction);	<- MACRO, equal to (lua_pushcfunction(l, f), lua_setglobal(L,n)))
...
//	fn lua_setenv(L: LuaState, index: c_int) -> c_int;
